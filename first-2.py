...
vlucht1_sec_30 = pd.read_excel('30Flight 1.xlsx')
vlucht2_sec_30 = pd.read_excel('30Flight 2.xlsx')
vlucht3_sec_30 = pd.read_excel('30Flight 3.xlsx')
vlucht4_sec_30 = pd.read_excel('30Flight 4.xlsx')
vlucht5_sec_30 = pd.read_excel('30Flight 5.xlsx')
vlucht6_sec_30 = pd.read_excel('30Flight 6.xlsx')
vlucht7_sec_30 = pd.read_excel('30Flight 7.xlsx')
...
print(pd.isna(vlucht7_sec_30).sum())


# In[244]:


vlucht1_sec_30 = vlucht1_sec_30.dropna()
vlucht2_sec_30 = vlucht2_sec_30.dropna()
vlucht3_sec_30 = vlucht3_sec_30.dropna()
vlucht4_sec_30 = vlucht4_sec_30.dropna()
vlucht5_sec_30 = vlucht5_sec_30.dropna()
vlucht6_sec_30 = vlucht6_sec_30.dropna()
vlucht7_sec_30 = vlucht7_sec_30.dropna()
print(pd.isna(vlucht6_sec_30).sum())
...
st.title("Voorspelling van vluchtvertraging")

# Laad de dataset (aangepast aan jouw kolommen)
df = merged_dataset  # Vervang door de juiste dataset

# Definieer een nieuwe kolom 'delayed' op basis van de kolom 'vertraging in (min)'
df['delayed'] = df['vertraging in (min)'].apply(lambda x: 1 if x > 15 else 0)

# Vul missing values in als dat nodig is
df.ffill(inplace=True)  # Gebruik ffill() in plaats van fillna()

# Omzetten van datums naar numerieke waarden (timestamps)
df['Geplande aankomst'] = pd.to_datetime(df['Geplande aankomst']).astype(int) // 10**9  # Zet om naar seconden
df['Werkelijke aankomst'] = pd.to_datetime(df['Werkelijke aankomst']).astype(int) // 10**9  # Zet om naar seconden

# Selecteer de relevante kenmerken (features)
X = df[['Vlucht nummer', 'Geplande aankomst', 'Werkelijke aankomst', 'ICAO', 'IATA', 'Aantal Wijzigingen Gate']]  # Vervang of voeg meer relevante kolommen toe
y = df['delayed']  # Doelvariabele: wel of geen vertraging

# Converteer categorische kolommen naar numeriek
X = pd.get_dummies(X, drop_first=True)

# Splits de dataset in train- en testdata (70% training, 30% testen)
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=42)

# Kies een RandomForestClassifier model
model = RandomForestClassifier()

# Train het model
model.fit(X_train, y_train)

# Maak voorspellingen met de testdata
y_pred = model.predict(X_test)

# Toon de nauwkeurigheid van het model
accuracy = accuracy_score(y_test, y_pred)
st.write(f"Model nauwkeurigheid: {accuracy * 100:.2f}%")

# Toon classificatierapport
st.subheader("Classificatierapport")
...
# Visualiseer de feature importances
fig2, ax2 = plt.subplots()
sns.barplot(x='Importance', y='Feature', data=feature_importances, ax=ax2)
plt.title('Feature Importances')
st.pyplot(fig2)

# Interactieve invoer voor voorspellingen
st.subheader("Voorspel of een vlucht vertraagd zal zijn")

# Gebruikersinput voor nieuwe voorspellingen
vlucht_nummer = st.text_input('Vlucht nummer')
geplande_aankomst = st.text_input('Geplande aankomst (bijv. 2024-10-09 14:30)')
werkelijke_aankomst = st.text_input('Werkelijke aankomst (bijv. 2024-10-09 14:50)')
icao_input = st.text_input('ICAO Code van de luchthaven (bijv. EHAM)')
iata_input = st.text_input('IATA Code van de luchthaven (bijv. AMS)')
gate_wijzigingen = st.number_input('Aantal Wijzigingen Gate', min_value=0)

# Maak een DataFrame van de gebruikersinput
user_input = pd.DataFrame({
    'Vlucht nummer': [vlucht_nummer],
    'Geplande aankomst': [geplande_aankomst],
    'Werkelijke aankomst': [werkelijke_aankomst],
    'ICAO': [icao_input],
    'IATA': [iata_input],
    'Aantal Wijzigingen Gate': [gate_wijzigingen]
})

# Omzetten van datums naar numerieke waarden (timestamps)
user_input['Geplande aankomst'] = pd.to_datetime(user_input['Geplande aankomst']).astype(int) // 10**9
user_input['Werkelijke aankomst'] = pd.to_datetime(user_input['Werkelijke aankomst']).astype(int) // 10**9

# Categorical features omzetten naar numeriek (net zoals in de trainingsset)
user_input = pd.get_dummies(user_input)
user_input = user_input.reindex(columns=X.columns, fill_value=0)

# Voorspelling doen op basis van de gebruikersinput
if st.button('Voorspel vertraging'):
    prediction = model.predict(user_input)
    if prediction[0] == 1:
        st.write("De vlucht zal waarschijnlijk vertraagd zijn.")
    else:
        st.write("De vlucht zal waarschijnlijk op tijd zijn.")
